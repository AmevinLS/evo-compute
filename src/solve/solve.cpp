...
std::map<heuristic_t, std::pair<std::string, std::string>> heuristic_t_str = {
    {RANDOM, {"random", "Random"}},
    {NN_END, {"nn_end", "NN (end)"}},
    {NN_ANY, {"nn_any", "NN (any)"}},
    {GREEDY_CYCLE, {"greedy_cycle", "Greedy Cycle"}},
    {GREEDY_CYCLE_REGRET, {"greedy_cycle_regret", "Greedy Cycle (w/ regret)"}},
    {GREEDY_CYCLE_REGRET_WEIGHTED,
     {"greedy_cycle_regret_weighted", "Greedy Cycle (w/ weighted regret)"}},
    {LOCAL_SEARCH_GEN_GREEDY_SWAP,
     {"local_search_gen_greedy_swap",
      "Local Search (heuristic, greedy, node swap)"}},
    {LOCAL_SEARCH_GEN_GREEDY_REVERSE,
     {"local_search_gen_greedy_reverse",
      "Local Search (heuristic, greedy, edge reverse)"}},
    {LOCAL_SEARCH_GEN_STEEPEST_SWAP,
     {"local_search_gen_steepest_swap",
      "Local Search (heuristic, steepest, node swap)"}},
    {LOCAL_SEARCH_GEN_STEEPEST_REVERSE,
     {"local_search_gen_steepest_reverse",
      "Local Search (heuristic, steepest, edge reverse)"}},
    {LOCAL_SEARCH_RANDOM_GREEDY_SWAP,
     {"local_search_random_greedy_swap",
      "Local Search (random, greedy, node swap)"}},
    {LOCAL_SEARCH_RANDOM_GREEDY_REVERSE,
     {"local_search_random_greedy_reverse",
      "Local Search (random, greedy, edge reverse)"}},
    {LOCAL_SEARCH_RANDOM_STEEPEST_SWAP,
     {"local_search_random_steepest_swap",
      "Local Search (random, steepest, node swap)"}},
    {LOCAL_SEARCH_RANDOM_STEEPEST_REVERSE,
     {"local_search_random_steepest_reverse",
      "Local Search (random, steepest, edge reverse)"}},
    {LOCAL_CANDIDATES, {"local_candidates", "Local Candidates"}},
};
...
std::map<heuristic_t, std::vector<solution_t> (*)(const tsp_t &, unsigned int)>
    random_heuristics_to_fn = {
        ...
        {LOCAL_CANDIDATES, solve_local_candidates},
        {LOCAL_DELTAS, solve_local_deltas_steepest_random},
